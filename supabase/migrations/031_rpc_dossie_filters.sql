-- ============================================================
-- 031_rpc_dossie_filters.sql
-- Adiciona filtros de duração, intervalo de datas e motivos de
-- término ao rpc_dossie_summary. Também retorna availableReasons
-- para popular o dropdown dinâmico no frontend.
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.rpc_dossie_summary(
  ...
  p_ended_reasons TEXT[]      DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  -- ── availableReasons: lista de motivos únicos para o filtro dinâmico ────────
  -- (ignora p_ended_reasons para mostrar todas as opções disponíveis no dropdown)
  SELECT COALESCE(
    jsonb_agg(er ORDER BY er),
    '[]'::JSONB
  ) INTO v_available_reasons
  FROM (
    SELECT DISTINCT ended_reason AS er
    FROM public.call_records_flat
    WHERE tenant_id    = p_tenant_id
      AND dial_queue_id = p_queue_id
  ...
  WITH calls AS (
    ...
    WHERE crf.tenant_id      = p_tenant_id
      ...
      AND (p_until IS NULL       OR crf.created_at      <= p_until)
      AND (p_min_duration IS NULL OR crf.duration_seconds >= p_min_duration)
      AND (p_max_duration IS NULL OR crf.duration_seconds <= p_max_duration)
      AND (p_ended_reasons IS NULL OR crf.ended_reason = ANY(p_ended_reasons))
  ),
  ov AS (
    SELECT
  ...
  funnel_raw AS (
    ...
    WHERE tenant_id    = p_tenant_id
      ...
      AND created_at   >= p_since
      AND (p_until IS NULL       OR created_at      <= p_until)
      AND (p_min_duration IS NULL OR duration_seconds >= p_min_duration)
      AND (p_max_duration IS NULL OR duration_seconds <= p_max_duration)
      AND (p_ended_reasons IS NULL OR ended_reason = ANY(p_ended_reasons))
  ...
  WITH base AS (
    SELECT * FROM public.call_records_flat
    WHERE tenant_id    = p_tenant_id
      AND dial_queue_id = p_queue_id
      AND created_at   >= p_since
      AND (p_until IS NULL       OR created_at      <= p_until)
  ...
  FROM (
    ...
    SELECT jsonb_build_object(
      ...
      'distribution', (SELECT COALESCE(jsonb_object_agg(v,c),'{}'::JSONB) FROM (SELECT interesse AS v,COUNT(*) AS c FROM base WHERE interesse IS NOT NULL GROUP BY interesse) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE interesse IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- resultado (enum)
    SELECT jsonb_build_object(
      'key','resultado','type','enum','count', cnt,
      'distribution', (SELECT COALESCE(jsonb_object_agg(v,c),'{}'::JSONB) FROM (SELECT resultado AS v,COUNT(*) AS c FROM base WHERE resultado IS NOT NULL GROUP BY resultado) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE resultado IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- nivel_engajamento (enum)
    SELECT jsonb_build_object(
      'key','nivel_engajamento','type','enum','count', cnt,
      'distribution', (SELECT COALESCE(jsonb_object_agg(v,c),'{}'::JSONB) FROM (SELECT nivel_engajamento AS v,COUNT(*) AS c FROM base WHERE nivel_engajamento IS NOT NULL GROUP BY nivel_engajamento) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE nivel_engajamento IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- objecao_principal (enum)
    SELECT jsonb_build_object(
      'key','objecao_principal','type','enum','count', cnt,
      'distribution', (SELECT COALESCE(jsonb_object_agg(v,c),'{}'::JSONB) FROM (SELECT objecao_principal AS v,COUNT(*) AS c FROM base WHERE objecao_principal IS NOT NULL GROUP BY objecao_principal) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE objecao_principal IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- cargo_presumido (enum)
    SELECT jsonb_build_object(
      'key','cargo_presumido','type','enum','count', cnt,
      'distribution', (SELECT COALESCE(jsonb_object_agg(v,c),'{}'::JSONB) FROM (SELECT cargo_presumido AS v,COUNT(*) AS c FROM base WHERE cargo_presumido IS NOT NULL GROUP BY cargo_presumido) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE cargo_presumido IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- qualidade_tecnica (enum)
    SELECT jsonb_build_object(
      'key','qualidade_tecnica','type','enum','count', cnt,
      'distribution', (SELECT COALESCE(jsonb_object_agg(v,c),'{}'::JSONB) FROM (SELECT qualidade_tecnica AS v,COUNT(*) AS c FROM base WHERE qualidade_tecnica IS NOT NULL GROUP BY qualidade_tecnica) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE qualidade_tecnica IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- score (number)
    SELECT jsonb_build_object(
      'key','score','type','number','count', cnt,
      'avg', (SELECT ROUND(AVG(score)::NUMERIC,1) FROM base WHERE score IS NOT NULL),
      'min', (SELECT MIN(score) FROM base WHERE score IS NOT NULL),
      'max', (SELECT MAX(score) FROM base WHERE score IS NOT NULL)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE score IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- performance_score (number)
    SELECT jsonb_build_object(
      'key','performance_score','type','number','count', cnt,
      'avg', (SELECT ROUND(AVG(performance_score)::NUMERIC,1) FROM base WHERE performance_score IS NOT NULL),
      'min', (SELECT MIN(performance_score) FROM base WHERE performance_score IS NOT NULL),
      'max', (SELECT MAX(performance_score) FROM base WHERE performance_score IS NOT NULL)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE performance_score IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- success_evaluation (boolean)
    SELECT jsonb_build_object(
      'key','success_evaluation','type','boolean','count', cnt,
      'trueCount',  (SELECT COUNT(*) FROM base WHERE success_evaluation = true),
      'falseCount', (SELECT COUNT(*) FROM base WHERE success_evaluation = false)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE success_evaluation IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- dor_identificada (text)
    SELECT jsonb_build_object(
      'key','dor_identificada','type','text','count', cnt,
      'samples', (SELECT COALESCE(jsonb_agg(s),'[]'::JSONB) FROM (SELECT DISTINCT dor_identificada AS s FROM base WHERE dor_identificada IS NOT NULL LIMIT 5) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE dor_identificada IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- resumo (text)
    SELECT jsonb_build_object(
      'key','resumo','type','text','count', cnt,
      'samples', (SELECT COALESCE(jsonb_agg(s),'[]'::JSONB) FROM (SELECT DISTINCT resumo AS s FROM base WHERE resumo IS NOT NULL LIMIT 5) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE resumo IS NOT NULL) t WHERE cnt > 0
    UNION ALL
    -- pontos_melhoria (text)
    SELECT jsonb_build_object(
      'key','pontos_melhoria','type','text','count', cnt,
      'samples', (SELECT COALESCE(jsonb_agg(s),'[]'::JSONB) FROM (SELECT DISTINCT pontos_melhoria AS s FROM base WHERE pontos_melhoria IS NOT NULL LIMIT 5) x)
    ) AS fa, cnt FROM (SELECT COUNT(*) AS cnt FROM base WHERE pontos_melhoria IS NOT NULL) t WHERE cnt > 0
  ) all_fields;
  ...
  SELECT jsonb_build_object(
    'interesse', COALESCE((
      ...
      FROM (SELECT interesse AS v, COUNT(*) AS cnt, ROUND(AVG(duration_seconds))::INT AS avg_dur
            ...
            WHERE interesse IS NOT NULL
              AND ended_reason IN ('customer-ended-call','assistant-ended-call')
    ...
    'nivel_engajamento', COALESCE((
      ...
      FROM (SELECT nivel_engajamento AS v, COUNT(*) AS cnt, ROUND(AVG(duration_seconds))::INT AS avg_dur
            FROM base
            WHERE nivel_engajamento IS NOT NULL
              AND ended_reason IN ('customer-ended-call','assistant-ended-call')
  ...
  v_result := v_result
    ...
    || jsonb_build_object('availableReasons',  v_available_reasons);
