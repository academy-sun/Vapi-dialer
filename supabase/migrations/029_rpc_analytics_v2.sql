-- ============================================================
-- 029_rpc_analytics_v2.sql
-- Evolução da rpc_analytics_summary:
--   • p_queue_ids UUID[]  — filtra por múltiplas filas (seleção por assistente)
--   • p_success_field TEXT — campo de sucesso configurado por assistente
--   • p_success_value TEXT — valor de sucesso esperado
-- Conversões agora usam outputs_flat dinâmicamente quando configurado,
-- em vez do campo pré-calculado is_conversion (que não respeita config por assistente)
-- ============================================================
...
BEGIN
  ...
  base_metrics AS (
    SELECT
      ...
      -- Conversões: usa outputs_flat dinâmico quando success_field configurado,
      -- senão cai no campo pré-calculado is_conversion
      COUNT(*) FILTER (WHERE
        CASE
          WHEN p_success_field IS NOT NULL
          THEN outputs_flat->>p_success_field = p_success_value
          ELSE is_conversion = true
        END
      )                                                  AS conversion_calls,
