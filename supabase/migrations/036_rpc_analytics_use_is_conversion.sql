-- ============================================================
-- 036_rpc_analytics_use_is_conversion.sql
-- Simplifica o cálculo de conversões na RPC de analytics:
-- usa o campo pré-calculado is_conversion (já gerado pelo trigger
-- trg_fn_flatten_call_record com fallback correto sobre sucesso=true)
-- em vez de re-consultar outputs_flat dinamicamente.
--
-- Motivação: o trigger já aplica o critério correto por assistente
-- (success_field/value do assistant_configs) e faz fallback para
-- aliases genéricos (sucesso:"true") quando o campo não existe.
-- A RPC antiga ignorava esse pré-cálculo e re-consultava o outputs_flat
-- pelo campo configurado globalmente, causando divergência entre o badge
-- na lista de chamadas (255) e o número no relatório (56).
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.rpc_analytics_summary(
  ...
  p_success_field TEXT    DEFAULT NULL,
  p_success_value TEXT    DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  calls AS (
    ...
    WHERE crf.tenant_id = p_tenant_id
      ...
      AND (p_queue_ids IS NULL OR crf.dial_queue_id = ANY(p_queue_ids))
  ),
  base_metrics AS (
    SELECT
      ...
      -- Usa is_conversion pré-calculado pelo trigger (inclui fallback correto
      -- para sucesso:"true" quando success_field não está preenchido no output)
      COUNT(*) FILTER (WHERE is_conversion = true)       AS conversion_calls,
      COUNT(*) FILTER (WHERE outputs_flat IS NOT NULL AND outputs_flat <> '{}'::jsonb) AS with_outputs_count,
  ...
  SELECT INTO v_result
    jsonb_build_object(
      ...
      'withOutputsCount',         bm.with_outputs_count,
